...
import { Moon, Sun, LogOut, LibrarySquare } from "lucide-react";
  ...
  return (
    <nav className="sticky top-0 z-50 w-full border-b border-border bg-background/80 backdrop-blur-md">
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 h-14 flex items-center justify-between">
        <Link href="/" className="flex items-center gap-2 font-bold text-[15px] sm:text-lg tracking-tight hover:opacity-80 transition-opacity whitespace-nowrap">
        ...
        <div className="flex items-center gap-3 sm:gap-4 shrink-0">
          ...
          {user ? (
            <div className="flex items-center gap-3 shrink-0">
              ...
              <div className="hidden sm:flex items-center gap-2 bg-muted/30 py-1.5 px-3 rounded-full border border-border/50 shrink-0">
                {user.user_metadata?.avatar_url ? (
                  ...
                  <div className="w-5 h-5 rounded-full bg-foreground text-background flex items-center justify-center text-[10px] font-bold shrink-0">
            ...
            <button onClick={handleLogin} className="flex items-center gap-2 sm:gap-2.5 bg-white text-gray-700 border border-gray-300 px-3 sm:px-4 py-2 rounded-xl text-[13px] sm:text-sm font-bold hover:scale-105 hover:bg-gray-50 transition-all shadow-sm shrink-0">
