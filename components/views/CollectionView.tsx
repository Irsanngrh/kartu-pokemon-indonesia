...
export default function CollectionView({ initialCollections, userName, isOwner, userId }: CollectionViewProps) {
  ...
  const handleShare = () => {
    const url = `${window.location.origin}/collection?uid=${userId}`;
      ...
      <header className="flex flex-col gap-1">
        <div className="flex items-center justify-between">
          ...
          {isOwner && (
            <button 
              onClick={handleShare}
              className="flex items-center gap-2 px-4 py-2 bg-foreground text-background rounded-xl text-sm font-bold hover:scale-105 transition-transform shadow-sm"
            >
