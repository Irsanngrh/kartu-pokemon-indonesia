...
export default function DeckBuilderView({ initialDeck }: { initialDeck: Deck | null }) {
  ...
  const fetchCatalog = async (pageToFetch: number, overwrite: boolean = false) => {
    ...
    // Merge catalog cards into fullCardObjects so deck panel can resolve card details.
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-0 relative -ml-4 sm:-ml-6 lg:-ml-8 w-[calc(100%+1rem)] sm:w-[calc(100%+1.5rem)] lg:w-[calc(100%+2rem)]">
      {toastMessage && (
        <div className={`fixed top-28 lg:top-20 left-1/2 -translate-x-1/2 z-[100] px-4 sm:px-6 py-3 rounded-2xl sm:rounded-full shadow-2xl text-xs sm:text-sm flex items-center gap-2.5 w-max max-w-[92vw] animate-in slide-in-from-top-4 ${toastMessage.type === 'error' ? 'bg-red-500 text-white' : 'bg-green-500 text-white'}`}>
      ...
      <div className="flex lg:hidden border-b border-border bg-background shrink-0">
        ...
        >
          Deck Saya
          {deckCards.length > 0 && (
            <span className="ml-1.5 bg-background dark:bg-muted/50 border border-border/50 text-foreground text-[10px] rounded-full px-1.5 py-0.5">{totalCardsInDeck}</span>
          )}
        </button>
      </div>
      ...
      <div className={`${mobileTab === 'catalog' ? 'flex' : 'hidden'} lg:flex flex-1 flex-col border-b lg:border-b-0 lg:border-r border-border/50 min-w-0`}>
        ...
        <div ref={headerRef} className="bg-background pt-4 shrink-0 flex flex-col">
          <div className="ml-4 sm:ml-6 lg:ml-8 mr-4 sm:mr-6 lg:mr-8 pb-2 flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="text-[10px] sm:text-[11px] text-foreground/40 tracking-widest uppercase mt-3 ml-1 font-medium">
              <span>Menampilkan {totalSearchCount} Kartu</span>
            </div>
          </div>
        </div>
        </div>
            ...
            {fetchedCards.map((card, cardIndex) => (
              <div key={`${card.id}_${cardIndex}`} className="relative group cursor-pointer" onClick={() => handleAddCard(card)}>
                ...
                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity rounded-lg flex items-center justify-center cursor-pointer">
                  <div className="bg-background dark:bg-muted/50 border border-border/50 text-foreground px-3 py-1.5 rounded-full text-xs flex items-center gap-1 shadow-lg transform translate-y-2 group-hover:translate-y-0 transition-all font-[inherit]">
                    <Plus size={14} /> Tambah
                  </div>
                </div>
              </div>
        ...
        {showScrollTop && (
          <div className="sticky bottom-4 lg:bottom-6 flex justify-end pr-4 sm:pr-6 lg:pr-12 pointer-events-none z-50 mt-[-50px]">
            <button
              ...
              className="p-4 bg-background dark:bg-muted/30 text-foreground rounded-full shadow-xl hover:scale-110 transition-transform pointer-events-auto border border-border/50"
      ...
      <div className={`${mobileTab === 'deck' ? 'flex' : 'hidden'} lg:flex lg:sticky lg:top-[64px] lg:self-start w-full lg:w-[341px] shrink-0 flex-col bg-background lg:max-h-[calc(100vh-64px)]`}>
        <div className="pl-4 lg:pl-4 pr-0 pt-4 pb-4 border-b border-border bg-background flex flex-col gap-4 shrink-0">
          ...
          <div className="flex items-center justify-between mr-0">
            ...
            <button
              ...
              disabled={isSaving}
              className={`flex items-center gap-2 px-4 py-2 text-sm rounded-xl font-[inherit] transition-all shadow-sm ${totalCardsInDeck > 60
                ? 'bg-muted/50 text-foreground/40 cursor-not-allowed'
                : 'bg-background border border-border/50 text-foreground hover:bg-muted/30 active:scale-95 cursor-pointer'
                }`}
            >
        ...
        <div className="flex-1 lg:overflow-y-auto lg:overflow-x-hidden pl-4 lg:pl-4 pr-0 pt-5 lg:pt-6 pb-6 lg:custom-scrollbar flex flex-col gap-5">
          <div className="flex flex-col gap-5 w-full lg:w-[calc(341px-1rem)] shrink-0">
          {deckCards.length === 0 ? (
            ...
            <>
              {["Pokemon", "Trainer", "Energi"].map(category => {
